package com.billow.core.system;
...
import com.billow.core.utils.BDebug;
import com.billow.core.utils.BString;
...
public class BSystem {
    ...
    public static String getSerialNumber(Context context){
        ...
        String s1 = UTDevice.getUtdid(context);
        if(s1.equals("unknown")||s1==null){
            s1 = getTempNumber(context);
        }
        if(s1.length()<1){
            s1 = getTempNumber(context);
        }

        return BString.toLowerCase(s1 ,0 ,s1.length()).replaceAll("[^a-zA-Z0-9]","");
    }
